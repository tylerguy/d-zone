...
function Inbox(config) {
  ...
  bot.on("ready", () => {
    ...
    for (let server of serverList) {
      ...
      if (server.ignoreChannels)
        newServer.ignoreChannels = server.ignoreChannels;
      if (server.listenChannels)
        newServer.listenChannels = server.listenChannels;
    ...
    bot.on(
      ...
      ({ author, member, channel, cleanContent: message }) => {
        ...
        if (!server) return;
        if (
          config.get("infoCommand") &&
          config.get("url") &&
          message === config.get("infoCommand")
        )
        ...
        if (
          ...
          return;
        if (
          server.ignoreUsers && // Check if this user is ignored
          server.ignoreUsers.indexOf(author.id) >= 0
        )
          return;
        if (
          server.ignoreChannels && // Check if this channel is ignored
          (server.ignoreChannels.indexOf(channel.name) >= 0 ||
            server.ignoreChannels.indexOf(channel.id) >= 0)
        )
          return;
        if (
          server.listenChannels && // Check if this channel is listened to
          server.listenChannels.indexOf(channel.name) < 0 &&
          server.listenChannels.indexOf(channel.id) < 0
        )
          return;
        self.emit("message", {
          type: "message",
...
Inbox.prototype.getUsers = function (connectRequest) {
  let server =
    this.servers.get(connectRequest.server) ||
    Array.from(this.servers.values()).find(
      (s) =>
        s.id === connectRequest.server ||
        (s.default && connectRequest.server === "default")
    );
  ...
  for (let [uid, member] of guild.members) {
    if (server.hideOffline && (!member.status || member.status === "offline"))
      continue;
